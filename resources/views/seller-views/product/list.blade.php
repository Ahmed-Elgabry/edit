                            ...
                            <table id="datatable"
                                ...
                                @php($category = DB::table('categories')->where('id', '=', $p->category_ids)->first())
                                    <tr>
                                        ...
                                        <td>
                                        <img src="{{asset('storage/app/public/product/thumbnail/'.$p->thumbnail)}}" alt="" width="100" height="100">
                                        </td>
                                        <td><a href="{{route('seller.product.view',[$p['id']])}}">
                                            @isset($p->name)
                                            {{$get_brand ? $get_brand->name : ''}} @isset($sub_sub_categories->id) {{$sub_sub_categories->name}} @endisset {{ $p->name }}
                                            @endisset

                                            @empty($p->name)
                                            {{$get_brand ? $get_brand->name : ''}} @isset($sub_sub_categories->id) {{$sub_sub_categories->name}} @endisset {{$p->product_type}}
                                            @endempty
                                            </a>
                                        </td>
