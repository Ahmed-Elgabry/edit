                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    @php($category = DB::table('categories')->where('id', '=', $product->category_ids)->first())
                                        <tr>
                                            <td>
                                            <img src="{{asset('storage/app/public/product/thumbnail/'.$product->images)}}" alt="" width="100" height="100">
                                            </td>
                                            
                                            @isset($product->name)
                                            <td>{{$get_brand? $get_brand->name : ''}} @isset($sub_sub_categories->id) {{$sub_sub_categories->name}}  @endisset {{ $product->name }}</td>
                                            @endisset

                                            @empty($product->name)
                                                <td>{{$get_brand? $get_brand->name : ''}} @isset($sub_sub_categories->id) {{$sub_sub_categories->name}}  @endisset {{$product->product_type}}</td>
                                            @endempty
