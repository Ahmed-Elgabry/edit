    ...
    <div class="col-lg-3 col-md-4 col-12">
        <div class="card">
            <div class="d-flex">
                <div class="d-flex justify-content-center align-items-center w-100">
                    <a href="{{ route('product.view', ['id' => $pro->id]) }}"
                        ...
                        <img src="{{ asset('storage/app/public/product/thumbnail/' . $pro->thumbnail) }}"
                            draggable="false" class="card-img-top " alt="{{ $pro->name }}"
                            style="width:100px;height:100px">
                    </a>
                </div>
                <div class="bg-light rounded-circle" id="wishlist{{ $pro->id }}"
                    style="width: 45px;height:40px;margin-left: 10px;margin-top: 10px;">
                    @if (auth('customer')->check())
                        @if (isset($get_pro_wishlist->id))
                            <button class="btn for-hover-bg removeWhishlist"
                                ...
                                <i class="fa fa-heart " style="color:#645cb3;font-size: 18px;"
                                    aria-hidden="true" id="heart-icon{{ $pro->id }}"></i>
                            </button>
                        @else
                            <button href="#" class="btn for-hover-bg addWhishlist"
                                data-product_id="{{ $pro->id }}">
                                <i class="fa fa-heart-o " style="color:#645cb3;font-size: 18px;"
                                    aria-hidden="true" id="heart-icon{{ $pro->id }}"></i>
                            </button>
                        @endif
                    @else
            ...
            <div class="card-body">
                ...
                @isset($pro->name)
                    <div class="pb-2">
                        <h5 class="card-title"
                            ...
                            <a href="{{ route('product.view', ['id' => $pro->id]) }}"
                                class="your-phone text-right" style="text-decoration:none;font-size:16px;">
                                {{ $Brand->name . ' ' . $pro->name }}
                ...
                <br>
                <div class="d-flex"
                    class="your-phone text-right pt-0" style="font-size:20px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">
                    <strong id="chosen_price">{{ $pro->unit_price * $pro->carton_unit }} </strong>
                    <strong class="mr-1"> ريال</strong>
                    <span class="mr-1" style="font-size:16px;font-weight: 600;">({{ $pro->unit_price }}
                        ريال للحبه)</span>
                </div>
